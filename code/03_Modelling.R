...
## Break out to separate script to test subsampling of absences and extract metrics ----
if (testing_subsampling & subsample_absences & !keep_all_cmip_vars) {
  cat("Testing subsampling of absences...\n")
  source("code/02_Exploratory/04_testing_absence_subsampling.R")
#   stop("Testing subsampling of absences; see code/02_Exploratory/04_testing_absence_subsampling.R for details.")
}

## Subsample absences to match presences ----
if (subsample_absences) {
  vme_n_pres <- sum(vme_df$VME_P_A == "Presence")
  set.seed(loop_seed)
  vme_df_for_prelimrf <- vme_df %>%
    group_by(VME_P_A) %>%
...
ggsave(plot_rf_prelim_var_imp, 
  filename = paste0("output/02_StepByStepOutputs/",
    vmeoi, "_", poi, "_", sspoi, "_SubsampAbs", subsample_absences,
    "_plot_rf_prelim_var_imp.png"), 
  width = 6, height = 4)
...
if (!keep_all_cmip_vars) {
  ...
  select_vme_vars <- function(pred_var_df, vmeoi, cor_threshold = 0.7, verbose = FALSE) {
    ...
    while (!vif_threshold_met) {
      ...
      corvif <- function(data, uncorrelated = uncor_vars, verbose = TRUE) {
  ...
  var_select_df[var_select_df$vmeoi == vmeoi & var_select_df$poi == poi & var_select_df$sspoi == sspoi & var_select_df$subsamp == subsample_absences, 
...
fold_metrics_df_i <- do.call(rbind, lapply(fold_metrics_list, function(x) {
  ...
  mutate(VME_Group = vmeoi, Period = poi, SSP = sspoi, Subsampled = subsample_absences, SelVars = keep_all_cmip_vars) %>%
  relocate(VME_Group, Period, SSP, Subsampled, SelVars, Fold, metric, value)
...
fold_metrics_summary_df_i <- fold_metrics_df_i %>%
  group_by(VME_Group, Period, SSP, Subsampled, SelVars, metric) %>%
...
raster_output_list[[paste(vmeoi, poi, sspoi, subsamp_title, selvars_title, sep = "_")]] <- rf_pred_stack
...
fold_var_imp_df <- lapply(fold_var_imp, function(fold) {
  ...
  ungroup()
write.csv(fold_var_imp_df, file = paste0("output/02_StepByStepOutputs/",
  paste(vmeoi, poi, sspoi, "table_rf_VarImp", subsamp_title, selvars_title, sep = "_"), ".csv"), row.names = FALSE)
...
ggsave(filename = paste0("output/02_StepByStepOutputs/",
  paste(vmeoi, poi, sspoi, "plot_rf_VarImp", subsamp_title, selvars_title, sep = "_"), ".png"), 
  width = 6, height = 4)
...
write.csv(fold_partial_df, file = paste0("output/02_StepByStepOutputs/",
  paste(vmeoi, poi, sspoi, "table_rf_PartialDep", subsamp_title, selvars_title, sep = "_"), ".csv"), row.names = FALSE)
...
ggsave(filename = paste0("output/02_StepByStepOutputs/",
   paste(vmeoi, poi, sspoi, "plot_rf_PartialDep", subsamp_title, selvars_title, sep = "_"), ".png"),
  width = 10, height = 8)
